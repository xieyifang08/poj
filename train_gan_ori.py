...
def main():
    ...
    for epoch in range(start_epoch, config["TRAIN"]["HYP"]["EPOCHS"]):
        ...
        # 处理并保存重建后的图像
        sample_png_dir = os.path.join(samples_dir, "picture_ori")
...
def build_model(
    ...
    g_model = model_ori.__dict__[config["MODEL"]["G"]["NAME"]](in_channels=config["MODEL"]["G"]["IN_CHANNELS"],
    ...
    d_model = model_ori.__dict__[config["MODEL"]["D"]["NAME"]](in_channels=config["MODEL"]["D"]["IN_CHANNELS"],
...
def define_loss(config: Any, device: torch.device) -> [nn.L1Loss, model_ori.ContentLoss, nn.BCEWithLogitsLoss]:
    ...
    if config["TRAIN"]["LOSSES"]["CONTENT_LOSS"]["NAME"] == "ContentLoss":
        feature_criterion = model_ori.ContentLoss(
...
def train(
        ...
        pixel_criterion: nn.L1Loss,
        content_criterion: model_ori.ContentLoss,
        adversarial_criterion: nn.BCEWithLogitsLoss,
